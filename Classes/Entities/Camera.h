...
public:
	...
	void rotate(Vector __v, float __degree) {
		...
		m._matrix[0][3] = (__current_position.x * (v * v + w * w) - u * (__current_position.y * v + __current_position.z * w) * (1 - cos(deg)) + (__current_position.y * w - __current_position.z * v) * sin(deg));
		m._matrix[1][3] = (__current_position.y * (u * u + w * w) - v * (__current_position.x * u + __current_position.z * w) * (1 - cos(deg)) + (__current_position.z * u - __current_position.x * w) * sin(deg));
		m._matrix[2][3] = (__current_position.z * (u * u + v * v) - w * (__current_position.x * u + __current_position.y * v) * (1 - cos(deg)) + (__current_position.x * v - __current_position.y * u) * sin(deg));

		__matrix_transformation = __matrix_transformation.multiplies(m);
